...
(* let buildLibrary ip liblistpath =
    ...
    (* refresh repo public_branch_anchor >>= fun () -> *)  (*this stops updating the public branch. Make this work after gc implementation*)
    
    ignore @@ build liblist private_branch_anchor (ip ^ "_private") repo ip;

    (* ignore @@ publish public_branch_anchor "127.0.0.1_private"; *) (*uncomment it to publish the changes of private branch *)
    (* ignore @@ publish private_branch_anchor "127.0.0.1_public"; *)

    (* Scylla_kvStore.get private_branch_anchor ["dune"] >>= fun lib -> Printf.printf "Count in dune = %d" lib.count; *)
    ...
    *)
    ...
    (* ignore @@ publish  public_branch_anchor private_branch_anchor; *)

    (* ignore @@ publish public_branch_anchor "127.0.0.1_private"; *)
    (* ignore @@ publish private_branch_anchor "127.0.0.1_public"; *)

    Scylla_kvStore.get public_branch_anchor ["dune"] >>= fun lib -> Printf.printf "Count in dune = %d" lib.count;

    Lwt.return_unit 

    (**the commented buildLibrary code is used to test the feasibility of private gc. *)
